...
inline router_state* get_router_state(struct sr_instance* sr) {//sr_instance在sr_base_internal.h文件中定义
	return (router_state*)sr->interface_subsystem;
}

/*init函数
输入：sr_instance，传入一个single router的实例
输出：无
功能：初始化了sr里面的一些变量，以及各种锁、互斥量，没有涉及到路由表、arp表之类的*/
 ...
 * --- CCDN，重要， 功能：从传进来的next_hop，查arp得到下一跳mac地址，然后转到send_packet函数
 */
