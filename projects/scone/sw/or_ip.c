...
void process_ip_packet(struct sr_instance* sr, const uint8_t * packet, unsigned int len, const char* interface) {
	...
	if (iface_match_ip(rs, (get_ip_hdr(packet, len))->ip_dst.s_addr)) {
	...
	else if( (get_ip_hdr(packet, len))->ip_p == IP_PROTO_CCCP ){
	    printf("scone has received a CCCP packet pos 1! \n");
        process_cccp_packet(sr, packet, len, interface);
	} else {
		/* Need to forward this packet to another host */
		struct in_addr next_hop; //in_addr用来表示一个32位的ipv4地址
		char next_hop_iface[IF_LEN]; //IF_LEN = 32
		bzero(next_hop_iface, IF_LEN);


		/* is there an entry in our routing table for the destination? */
		if(get_next_hop(&next_hop, next_hop_iface, IF_LEN,rs,&((get_ip_hdr(packet, len))->ip_dst))) {// true means no match
		...
		else {// else means match

            //若interface存的是从哪个口进的话，这就表示从进来的口又发出去？然后发icmp表示不可达？
