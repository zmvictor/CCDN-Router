...
int process_icmp_echo_reply_packet(struct sr_instance* sr, const uint8_t* packet, unsigned int len) {
	...
	/* Create an entry in the sping queue for this echo reply, sping queue is a struct, defined in data_types.h, include packet, len and arrival_time */
	//from wiki:Sping is short for "spam ping", and is related to fraudulent pings from blogs using trackbacks, called trackback spam.
