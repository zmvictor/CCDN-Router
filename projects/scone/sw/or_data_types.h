...
// --- CCDN
/** CCCP HEADER STRUCTURE **/
struct cccp_hdr
{
	uint8_t cccp_type;				/* type of cccp packet */
	uint8_t cccp_identifier;			/* identifier */
	uint16_t cccp_checksum;				/* checksum */
	struct in_addr cccp_content;	/* content name field */
	uint16_t cccp_vn;               /* version number */
} __attribute__ ((packed)) ;
typedef struct cccp_hdr cccp_hdr;

#define CCCP_TYPE_REQUEST  0x00
#define CCCP_TYPE_REPLY    0x01
#define CCCP_TYPE_FINISH   0x03
#define CCCP_TYPE_REJECT   0x02
...
typedef struct ctable_entry ctable_entry;
#define MAX_CTABLE_DEPTH 32
